...
try:
    ...
    BUILDER_CLIENT_AVAILABLE = True
except ImportError:
    BUILDER_CLIENT_AVAILABLE = False
    BuilderRelayerClient = None
import config
        ...
        try:
            if BUILDER_CLIENT_AVAILABLE:
                try:
                    ...
                    logger.warning(f"⚠️ Could not initialize Builder Relayer Client: {e}")
                    self.relayer_client = None
            else:
                self.relayer_client = None
                logger.warning("⚠️ Builder Relayer Client not available (web3 not installed) - merge/redeem disabled")
        except Exception as e:
